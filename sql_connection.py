...
def setup_database():
    ...
    create_statements = [
        ...
        CREATE TABLE Pharmacy_Order (
            ...
            CONSTRAINT fk_order_patient FOREIGN KEY (PatientID) REFERENCES UserAccount(UserID)
        )
        """,
        """
        ...
        """
        CREATE TABLE Bill (
            BillID INTEGER IDENTITY(1,1) PRIMARY KEY,
    ...
    Pharmacy_Order_data = [
        (1, 1, 6, 10, 5.00),      # OrderID, ItemID, PatientID, Quantity, Price
        (2, 2, 7, 5, 6.50),
        (3, 3, 8, 3, 12.00),
        (4, 4, 9, 2, 18.00),
        (5, 5, 10, 4, 10.00),
    ...
    Pharmacy_Order_insert_query = """
        INSERT INTO Pharmacy_Order (OrderID, ItemID, PatientID, ItemQuantity, PricePerItem)
        VALUES (?, ?, ?, ?, ?)
    """
    ...
    Bill_data = [
        (1, 6, '2025-10-28', 50.00, 'Paid'),      # OrderID, PatientID, BillDate, TotalPrice, BillStatus
        (2, 7, '2025-10-28', 32.50, 'Paid'),
        (3, 8, '2025-10-28', 36.00, 'Paid'),
        (4, 9, '2025-10-28', 36.00, 'Paid'),
        (5, 10, '2025-10-28', 40.00, 'Paid'),
        (6, 11, '2025-10-28', 48.00, 'Paid'),
        (7, 12, '2025-10-28', 37.50, 'Paid'),
        (8, 13, '2025-10-28', 27.00, 'Paid'),
        (9, 14, '2025-10-28', 50.00, 'Paid'),
        (10, 15, '2025-10-28', 55.00, 'Paid'),
        (11, 16, '2025-10-29', 30.00, 'Unpaid'),
        (12, 17, '2025-10-29', 45.00, 'Unpaid'),
        (13, 18, '2025-10-29', 48.00, 'Unpaid'),
        (14, 19, '2025-10-29', 36.00, 'Unpaid'),
        (15, 20, '2025-10-29', 50.00, 'Unpaid'),
        (16, 6, '2025-10-30', 30.00, 'Unpaid'),
        (17, 7, '2025-10-30', 40.00, 'Unpaid'),
        (18, 8, '2025-10-30', 50.00, 'Unpaid'),
        (19, 9, '2025-10-30', 36.00, 'Unpaid'),
        (20, 10, '2025-10-30', 56.00, 'Unpaid')
    ]

    # Use IDENTITY_INSERT to insert with specific IDs for sample data
    cursor.execute("SET IDENTITY_INSERT Bill ON")
    ...
    # Create the data with BillIDs
    bill_data_with_ids = [(i+1, row[0], row[1], row[2], row[3], row[4]) for i, row in enumerate(Bill_data)]
    cursor.executemany(Bill_insert_query, bill_data_with_ids)

    cursor.execute("SET IDENTITY_INSERT Bill OFF")
    connection.commit()
